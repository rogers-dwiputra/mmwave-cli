...
For framing_type = 1 (finite), the radar stops automatically after nframes_master frames,
so capture_time only needs to be a safe upper bound.
--]]

capture_time                     =   12000                           -- ms  (12s timeout > 200 × 50ms = 10s)
...
capture_directory               =   "GT_static_4m_10s"
num_frames_to_capture           =   0                       -- 0: default case; Any positive value - number of frames to capture

framing_type                    =   1                       -- 0: infinite, 1: finite (auto-stops after nframes_master)
...
capture_directory_datetime = capture_directory.."_"..datetime
status = ar1.TDACaptureCard_StartRecord_mult(1, n_files_allocation, data_packaging, capture_directory_datetime, num_frames_to_capture)
if (status == 0) then
...
if (framing_type == 0) then
    ...
    -- Stop capturing (only needed for infinite mode)
